      ...
      <Card className="shadow-lg border-2">
        <CardContent className="pt-6">
          <div className="flex flex-col items-center justify-center py-12 text-muted-foreground">
            ...
            <p className="text-sm mt-1">Add products by entering their barcode above</p>
          </div>
        </CardContent>
      </Card>
    );
            ...
            {items.map((item) => (
              ...
              >
                <div className="flex-1">
                  <h3 className="font-semibold text-base">{item.name || "Unknown Product"}</h3>
                  <p className="text-sm text-muted-foreground mt-1">
                    Barcode: {item.code}
                  </p>
                  <div className="flex gap-4 mt-2 text-xs text-muted-foreground">
                    <span>Carbs: {(item.nutriments.carbohydrates || 0).toFixed(1)}g</span>
                    <span>Protein: {(item.nutriments.proteins || 0).toFixed(1)}g</span>
                    <span>Fat: {(item.nutriments.fat || 0).toFixed(1)}g</span>
